...
**Why this is a research pool, not a candidate roster.** The trajectory layer (Stage 9) is anchor-based — it assumes a single dominant title and classifies the studio by its anchor share, anchor age, and late-vs-early-half owners split. Diversified studios are, by definition, below the 70% anchor-share threshold required for any `ANCHOR_*` label, so the Stage 11 gate (`RISING` or `ANCHOR_RECENT`) is structurally unfair to them.

**The run was nonetheless executed for transparency.** Re-running Stages 8–11 on the 108-dev pool reusing the same code as the main funnel yields: 67 `FALLING`, 30 `STEADY`, 11 `RISING`, and **zero** `ANCHOR_*` labels (impossible by construction). After re-applying the suspect-battery `CLEAR` filter at Stage 11, **9 research candidates** surface — all `RISING`+`CLEAR`. These should be read as an *over-qualified subset*, not a complete answer: a portfolio-appropriate momentum metric (catalog-weighted late-vs-early, catalog expansion rate, genre-spread cadence) would surface different and likely more numerous candidates. The 9-candidate list lives in the private repo at `research_pool/research_pool_candidates.csv` alongside `research_pool_full_scored.csv` with all four layers (PCI, suspect, trajectory, country).
